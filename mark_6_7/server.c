...
int main(int argc, char *argv[]) {
    ...
    *counter = 0;
    ...
    if (argc != 5) {
        printf("Usage: <%s> <port> <areas number> <clients number> <listeners number>\n", argv[0]);
        exit(1);
    }
    int areas_number = atoi(argv[2]);
    int clients_number = atoi(argv[3]);
    int listeners_number = atoi(argv[4]);
    ...
    int listenerSockets[MAX_CLIENTS];
    ...
    for (int i = 0; i < listeners_number; ++i) {
        int newListenerSocket = accept(serverSocket, (struct sockaddr *) &clientAddr, &clientAddrLen);
        if (newListenerSocket < 0) {
            perror("Ошибка при принятии соединения");
            exit(1);
        }
        listenerSockets[i] = newListenerSocket;
        printf("client is connected %d\n", newListenerSocket);
...
//                lands[j + 1] = 0;
            ...
            while (msg_p->type != MSG_TYPE_FINISH && msg_p->type != MSG_TYPE_EMPTY) {
                ...
                if (strcmp(buffer_2, "Treasure is found by team") == 0) {
                    ...
                    for (int j = 0; j < listeners_number; ++j) {
                        if (send(listenerSockets[j], "WE HAVE A NEW WINNER", sizeof("WE HAVE A NEW WINNER"), 0) < 0) {
                            perror("Ошибка при отправке сообщения на сервер");
                            exit(1);
                        }
                        if (send(listenerSockets[j], buffer, sizeof(buffer), 0) < 0) {
                            perror("Ошибка при отправке сообщения на сервер");
                            exit(1);
                        }
                    }
                    ...
                    *counter = *counter + 1;
                    if (*counter == clients_number) {
                        ...
                        msg_p->type = MSG_TYPE_FINISH;
                        for (int j = 0; j < listeners_number; ++j) {
                            if (send(listenerSockets[j], "NOTHING WAS FOUND", sizeof("NOTHING WAS FOUND"), 0) < 0) {
                                perror("Ошибка при отправке сообщения на сервер");
                                exit(1);
                            }
                        }
            ...
            if (msg_p->type != MSG_TYPE_EMPTY) {
                if (is_treasure_found) {
                    ...
                    for (int j = 0; j < listeners_number; ++j) {
                        if (send(listenerSockets[j], "Treasure was not found", sizeof("Treasure was not found"), 0) <
                            0) {
                            perror("Ошибка при отправке сообщения на сервер");
                            exit(1);
                        }
    ...
    for (int j = 0; j < listeners_number; ++j) {
        if (listenerSockets[j] != -1) {
            if (send(listenerSockets[j], "The End", sizeof("The End"), 0) < 0) {
                perror("Ошибка при отправке сообщения на сервер");
                exit(1);
            }
        }
